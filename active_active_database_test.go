...
func TestAADatabase_Create(t *testing.T) {
  ...
  "_links": {
	...
	actual, err := subject.Database.ActiveActiveCreate(context.TODO(), 42, databases.CreateActiveActiveDatabase{
		...
		Replication:                         redis.Bool(true),
		LocalThroughputMeasurement: &databases.CreateLocalThroughputMeasurement{
			Region:                   "",
		...
		GlobalPassword:         redis.String("fooBar"),
		Alerts: []*databases.Alert{
			{
				Name:  redis.String("dataset-size"),
				Value: redis.Int(80),
